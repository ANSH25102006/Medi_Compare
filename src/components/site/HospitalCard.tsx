  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-soft hover-card-lift">
      <div className="relative h-44 overflow-hidden">
        ...
        />
        {/* Rating Badge */}
        <div className="absolute left-3 top-3 flex items-center gap-1 rounded-full bg-background/95 px-2.5 py-1 text-xs font-semibold shadow-soft backdrop-blur border border-border/30">
        ...
        <button
          ...
          className="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-full bg-background/95 text-muted-foreground shadow-soft transition-all hover:scale-110 active:scale-95 cursor-pointer z-10 border border-border/30"
        >
          <Heart
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mt-5 rounded-xl bg-primary-soft/50 border border-primary/10 p-3.5 shadow-inner dark:bg-primary-soft/5">
          <div className="flex items-center justify-between">
            ...
            {isDeal && (
              <span className="inline-flex items-center gap-0.5 rounded-full bg-success/15 px-2 py-0.5 text-[10px] font-bold text-success border border-success/20">
                <BadgeCheck className="h-3.5 w-3.5" /> Best value
              </span>
            )}
          </div>
          ...
          {isDeal && (
            <p className="mt-2 text-[11px] font-semibold text-success flex items-center gap-1">
              ✓ Save {Math.round((savings / avg) * 100)}% over market average
            </p>
          )}
        </div>
        ...
        <div className="mt-5 flex gap-2 pt-1">
          <Button asChild variant="outline" className="flex-1 rounded-full btn-interactive">
