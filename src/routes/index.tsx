...
function Landing() {
  ...
  return (
    <SiteShell>
      <section className="relative overflow-hidden bg-hero-gradient min-h-[620px] md:min-h-[720px] flex items-center">
        {/* Layered Lighting & Blurred Ambient Elements */}
        <div className="absolute -left-1/4 -top-1/4 w-[60%] h-[60%] rounded-full bg-[radial-gradient(circle,rgba(59,130,246,0.18)_0%,transparent_70%)] blur-3xl pointer-events-none" />
        <div className="absolute -right-1/4 -bottom-1/4 w-[60%] h-[60%] rounded-full bg-[radial-gradient(circle,rgba(34,197,94,0.1)_0%,transparent_70%)] blur-3xl pointer-events-none" />
        <div className="absolute left-[35%] top-[10%] w-[30%] h-[30%] rounded-full bg-[radial-gradient(circle,rgba(147,51,234,0.08)_0%,transparent_60%)] blur-3xl pointer-events-none animate-pulse duration-[7000ms]" />

        {/* Subtle grid pattern overlay */}
        <div className="absolute inset-0 z-0 opacity-[0.04] dark:opacity-[0.03] bg-[linear-gradient(to_right,oklch(0.56_0.17_250)_1px,transparent_1px),linear-gradient(to_bottom,oklch(0.56_0.17_250)_1px,transparent_1px)] bg-[size:40px_40px] pointer-events-none" />

        <div className="relative z-10 mx-auto grid max-w-7xl gap-12 px-4 py-24 sm:px-6 md:py-32 lg:grid-cols-2 lg:px-8 w-full items-center">
          {/* Left Side Content */}
          <div className="animate-fade-in flex flex-col justify-center relative">
            <div className="flex w-fit items-center gap-2 rounded-full border border-primary/20 bg-primary/8 px-4 py-1.5 text-[13px] font-bold text-primary backdrop-blur-md shadow-sm transition-all hover:bg-primary/12 hover:shadow-md cursor-default relative z-10">
              <Sparkles className="h-4 w-4 text-primary animate-pulse" />
              Trusted by 50,000+ patients
            </div>

            <h1 className="mt-8 text-[3rem] font-black leading-[1.02] tracking-tighter text-foreground md:text-6xl lg:text-[4.75rem] relative z-10">
            ...
            <p className="mt-7 max-w-xl text-lg leading-relaxed text-muted-foreground font-medium sm:text-xl relative z-10 text-balance">
            ...
            <form
              ...
              className="mt-10 glass flex flex-col gap-2 rounded-2xl p-2.5 shadow-[0_20px_50px_rgba(0,72,206,0.08)] sm:flex-row sm:items-center relative z-25 border border-white/40 dark:border-border/30"
            >
              ...
              <Button
                type="submit"
                size="lg"
                className="rounded-xl h-[52px] px-8 font-bold text-base bg-primary-gradient hover:shadow-lg transition-all btn-interactive cursor-pointer"
              >
                Search
              </Button>
            </form>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2 relative z-20">
              ...
              {[
                ...
                <button
                  ...
                  className="rounded-full border border-border bg-card/60 px-3.5 py-1.5 text-xs font-semibold text-muted-foreground hover:bg-primary/5 hover:text-primary hover:border-primary/20 transition-all duration-200 cursor-pointer shadow-sm active:scale-95 btn-interactive"
                >
                  {term}
                </button>
              ))}
      ...
      <section className="bg-secondary/30 py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className="text-sm text-muted-foreground mt-3 max-w-lg mx-auto font-medium leading-relaxed">
          ...
          <ol
            ref={howItWorksRef}
            className="relative mt-16 grid gap-6 sm:grid-cols-2 md:grid-cols-4"
          >
            ...
            {stepsData.map((s, idx) => (
              <ScrollReveal key={s.n} delay={idx * 80}>
                <li className="group relative rounded-3xl border border-border bg-card p-6 shadow-soft hover-card-lift flex flex-col justify-between h-full">
                  <div>
        ...
        <div className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((f, idx) => (
            <ScrollReveal key={f.title} delay={idx * 80}>
              <div className="group rounded-3xl border border-border bg-card p-8 shadow-soft hover-card-lift h-full">
          ...
          <div className="mt-16 grid gap-6 md:grid-cols-3">
            {homeTestimonials.map((t, idx) => (
              <ScrollReveal key={t.name} delay={idx * 80}>
                <figure className="rounded-3xl border border-border bg-card p-8 shadow-soft relative overflow-hidden flex flex-col justify-between hover-card-lift h-full">
                  <div>
        ...
        <Accordion type="single" collapsible className="mt-10 space-y-3">
          {faqs.map((f, i) => (
            <AccordionItem
              key={f.q}
              value={`item-${i}`}
              className="rounded-2xl border border-border bg-card px-5 shadow-soft hover:border-primary/20 transition-all duration-300"
            >
      ...
      <section className="mx-auto max-w-7xl px-4 pb-24 sm:px-6 lg:px-8">
        <div className="relative overflow-hidden rounded-3xl bg-primary-gradient p-12 text-primary-foreground shadow-elevated md:p-20">
          ...
          <div className="relative grid items-center gap-8 md:grid-cols-2">
            <div>
              <h2 className="text-3xl font-extrabold leading-tight md:text-4xl text-balance tracking-tight">
                Ready to take control of your healthcare?
              </h2>
              <p className="mt-3 max-w-md text-primary-foreground/80 font-medium">
            ...
            <div className="flex flex-wrap gap-3 md:justify-end">
              ...
              <Button
                ...
                size="lg"
                variant="outline"
                className="rounded-full border-white/30 bg-transparent text-primary-foreground hover:bg-white/10 hover:text-primary-foreground btn-interactive"
              >
